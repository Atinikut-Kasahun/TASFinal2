...
export default function LoginOverlay({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const roleSlug = (typeof firstRole === 'string' ? firstRole : firstRole?.slug || firstRole?.name || 'ta_manager').toLowerCase();
            onClose();
            if (roleSlug === 'admin') {
            ...
            {isOpen && (
                <>
                    ...
                    <div className="fixed inset-0 z-[70] overflow-y-auto flex items-center justify-center p-4 sm:p-6">
                        ...
                        >
                            <div className="relative bg-white/[0.85] backdrop-blur-[32px] rounded-[32px] sm:rounded-[48px] shadow-[0_32px_80px_rgba(0,0,0,0.15)] border border-white/50 p-6 sm:p-10 overflow-hidden group">
                                ...
                                <button
                                    onClick={onClose}
                                    className="absolute top-5 right-5 sm:top-8 sm:right-8 p-2 rounded-full hover:bg-black/5 transition-colors text-black/40 hover:text-black z-20"
                                >
                                    <X size={20} />
                                </button>

                                <div className="text-center mb-7 sm:mb-10 relative z-10">
                                    <div className="inline-flex items-center justify-center p-3 sm:p-4 bg-[#FDF22F] rounded-2xl mb-4 sm:mb-6 shadow-lg shadow-[#FDF22F]/20 rotate-3 group-hover:rotate-0 transition-transform duration-500">
                                        <ShieldCheck size={28} className="text-black" strokeWidth={2.5} />
                                    </div>
                                    <h1 className="text-3xl sm:text-4xl font-black text-black tracking-tighter mb-2">LOGIN</h1>
                                    <div className="h-1 w-12 bg-[#FDF22F] mx-auto rounded-full mb-3 sm:mb-4" />
                                    <p className="text-[13px] sm:text-[14px] font-medium text-black/40">Secure access to Hiring Intelligence</p>
                                </div>
                                ...
                                <AnimatePresence>
                                    {error && (
                                        <motion.div
                                            ...
                                            className="mb-6 sm:mb-8 p-3 bg-red-50 border-l-4 border-red-500 text-red-600 text-[11px] font-black uppercase tracking-widest rounded-r-xl overflow-hidden"
                                        >
                                ...
                                <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8 relative z-10">
                                    ...
                                    <div className="flex items-center justify-between pt-2">
                                        <button
                                            type="submit"
                                            disabled={loading}
                                            className="relative flex items-center gap-3 bg-black text-white px-8 sm:px-10 py-3.5 sm:py-4 rounded-2xl font-black uppercase tracking-[0.2em] text-[12px] hover:bg-[#FDF22F] hover:text-black transition-all group/btn overflow-hidden shadow-xl shadow-black/10 active:scale-95 disabled:opacity-50"
                                        >
                                ...
                                <div className="mt-8 sm:mt-12 pt-6 sm:pt-8 border-t border-black/5 relative z-10">
                                    <p className="text-center text-[9px] font-black text-black/15 uppercase tracking-[0.4em] mb-4 sm:mb-6">Speed Pass Portal</p>
                                    <div className="grid grid-cols-4 gap-2 sm:gap-3">
                                        {[
                                            ...
                                            <button
                                                ...
                                                className="flex flex-col items-center gap-1.5 sm:gap-2 p-2 sm:p-3 rounded-2xl bg-black/[0.02] border border-black/[0.03] hover:bg-[#FDF22F] hover:border-[#FDF22F] transition-all group/acc"
                                            >

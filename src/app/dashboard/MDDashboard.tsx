...
export default function MDDashboard({ user, activeTab: initialTab, onLogout }: { user: any; activeTab: string; onLogout: () => void }) {
    ...
    return (
        <div className="relative bg-[#F5F6FA] pb-20 -mx-8 min-h-screen">
            {/* Full-Width Yellow Banner (covers top portion) */}
            <div className="absolute top-0 left-0 right-0 h-[220px] bg-[#FDF22F] z-0 shadow-[0_2px_4px_rgba(0,0,0,0.1),_0_20px_40px_-4px_rgba(0,0,0,0.15)]" />

            <div className="relative z-10 px-8 pt-10">
                {/* Page Title Row */}
                <div className="flex justify-between items-center mb-8">
                    <div className="flex items-center gap-4">
                        {localTab === 'HOME' && <Home size={32} strokeWidth={2.5} className="text-black" />}
                        {localTab === 'JOBS' && <Briefcase size={32} strokeWidth={2.5} className="text-black" />}
                        {localTab === 'HIRING PLAN' && <ClipboardList size={32} strokeWidth={2.5} className="text-black" />}
                        {localTab === 'REPORTS' && <BarChart2 size={32} strokeWidth={2.5} className="text-black" />}
                        <h1 className="text-5xl font-black italic tracking-tighter text-black">
                    ...
                    {localTab === 'HIRING PLAN' && selectedIds.length > 0 && (
                        <button
                            ...
                            className="bg-black text-[#FDF22F] px-8 py-3.5 rounded-xl font-black text-[13px] tracking-widest uppercase flex items-center gap-2"
                        >
                            <Check size={16} />
                ...
                <div className="flex gap-6 items-start">
                    ...
                    {/* ── Left: Profile Card ── */}
                    <div className="w-[300px] shrink-0 bg-white overflow-hidden shadow-sm border border-gray-100">
                        {/* Yellow top: avatar + name */}
                        ...
                        <div className="px-6 py-6 space-y-5">
                            ...
                            <div className="flex gap-3 pt-2">
                                <button 
                                    ...
                                    className="w-12 h-12 bg-gray-100 rounded-xl flex items-center justify-center text-gray-500 hover:bg-black hover:text-[#FDF22F] transition-colors"
                                >
                                    <Key size={18} strokeWidth={2.5} />
                                </button>
                                <button
                                    onClick={onLogout}
                                    className="flex-1 h-12 flex items-center justify-center gap-2 bg-red-50 text-red-500 font-bold text-xs tracking-widest uppercase rounded-xl hover:bg-red-100 transition-colors border border-red-100"
                                >
                                ...
                                <button
                                    ...
                                    title="Hiring & Availability Calendar"
                                    className="h-12 w-12 flex items-center justify-center bg-gray-100 text-gray-500 font-bold rounded-xl hover:bg-black hover:text-[#FDF22F] transition-colors border border-gray-100 shrink-0"
                                >
                                    <Calendar size={18} />
                                </button>
                            </div>
                    ...
                    {/* ── Right: Content Card ── */}
                    <div className="flex-1 bg-white overflow-hidden shadow-sm border border-gray-100">
                        {/* Yellow label bar */}
                        <div className="bg-[#FDF22F] px-6 py-3 flex items-center justify-between">
                            ...
                            <>
                                {localTab === 'HOME' && (
                                    <div className="p-10 space-y-4">
                                        <h2 className="text-2xl font-black">Welcome, {user?.name || 'Managing Director'}</h2>
                                    ...
                                    <div className="flex flex-col w-full">
                                        ...
                                        <div className="grid grid-cols-3 divide-x divide-gray-100 border-b border-gray-100 bg-white">
                                            <div className="px-6 py-4">
                                                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-0.5">Total Requisitions</p>
                                                <p className="text-2xl font-black text-black tracking-tighter">{reqsMeta?.total || requisitions.length}</p>
                                            </div>
                                            <div className="px-6 py-4">
                                                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-0.5">Pending Approval</p>
                                                <p className="text-2xl font-black text-amber-500 tracking-tighter">{pendingReqs.length}</p>
                                            </div>
                                            <div className="px-6 py-4">
                                                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-0.5">Approved</p>
                                                <p className="text-2xl font-black text-emerald-500 tracking-tighter">{requisitions.filter(r => r.status === 'approved').length}</p>
                                            </div>
                                        </div>
                                        <table className="w-full text-left">
                                            <thead className="border-b border-gray-100">
                                        ...
                                        <tbody className="divide-y divide-gray-50">
                                            {requisitions.length === 0 ? (
                                                ...
                                                <tr key={req.id} className="hover:bg-gray-50 transition-colors cursor-pointer">
                                                    ...
                                                    <td className="px-6 py-5 text-[13px] text-black font-black tabular-nums">
                                                        {req.budget ? req.budget.toLocaleString() : '1,200'} ETB
                                                    </td>
                                                    <td className="px-6 py-5 text-[12px] text-gray-500">
                                                        {req.created_at ? new Date(req.created_at).toLocaleDateString('en-US', { month: 'numeric', day: 'numeric', year: 'numeric' }) : '—'}
                                                    </td>
                                                    <td className="px-6 py-5">
                                                        {canApprove(req) ? (
                                                            <div className="flex gap-2">
                                                                ...
                                                                {isMD && (
                                                                    <button onClick={(e) => { e.stopPropagation(); setFeedbackTarget(req.id); setActionType('amend'); setFeedbackReason(''); setDrawerReq(req); }} className="text-[10px] font-black text-amber-600 bg-amber-50 px-4 py-1.5 rounded-lg hover:bg-amber-100 transition-all uppercase tracking-widest">Amend</button>
                                                                )}
                                                            ...
                                                            <span className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest ${
                                                                ...
                                                                (req.status === 'pending_md' || req.status === 'pending_hr') ? 'bg-yellow-50 text-amber-700' :
                                                                'bg-gray-100 text-gray-500'
                                                            }`}>
                                                                {req.status === 'pending_md' || req.status === 'pending_hr' ? 'Pending' : req.status.replace('_', ' ')}
                                                            </span>
                                                        )}
                                                    </td>
                                                </tr>
                                    ...
                                    <div className="p-8 space-y-8">
                                        ...
                                        <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 flex items-center justify-between">
                                            <div className="flex items-center gap-6">
                                                ...
                                                <div>
                                                    ...
                                                    <select value={reportFilters.jobId} onChange={e => setReportFilters(prev => ({ ...prev, jobId: e.target.value }))} className="text-sm font-bold text-black bg-transparent outline-none cursor-pointer w-40">
                                            ...
                                            <div className="flex gap-3">
                                                <button onClick={() => fetchData()} className="px-4 py-2 bg-white text-gray-600 rounded-lg text-xs font-bold hover:bg-gray-100 transition-colors border border-gray-200">↻ Refresh</button>
                                                <button
                                                    onClick={() => {
                                                    ...
                                                    className="px-5 py-2 bg-[#FDF22F] text-black hover:bg-black hover:text-[#FDF22F] rounded-xl text-[11px] font-black uppercase tracking-widest transition-all flex items-center gap-2"
                                                >
                                        ...
                                        <div className="grid grid-cols-4 gap-5">
                                            {[
                                                ...
                                                <div key={i} className="bg-gray-50 p-5 rounded-2xl border border-gray-100 hover:scale-[1.02] transition-transform">
                        ...
                        <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="fixed right-0 top-0 bottom-0 w-full max-w-xl bg-white shadow-2xl z-[120] overflow-y-auto">
                            <div className="p-8 border-b border-gray-100 flex justify-between items-start">
                                <div>
                                    ...
                                    <h2 className="text-2xl font-black text-black">{drawerReq.title}</h2>
                            ...
                            <div className="p-8 space-y-8">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="bg-gray-50 p-5 rounded-xl border border-gray-100 border-dashed max-h-[300px] overflow-y-auto">
                                    ...
                                    <div className="pt-8 space-y-4 border-t border-gray-100">
                                        {feedbackTarget === drawerReq.id ? (
                                            ...
                                            <div className="flex gap-3">
                                                ...
                                                {isMD && (
                                                    <button onClick={() => { setFeedbackTarget(drawerReq.id); setActionType('amend'); }} className="flex-1 px-6 py-5 bg-amber-50 text-amber-600 rounded-[20px] text-[12px] font-black tracking-widest uppercase border border-amber-100 hover:border-amber-500 transition-all">Amend</button>
                                                )}
            ...
            <AnimatePresence>
                {isPasswordModalOpen && (
                    <div className="fixed inset-0 z-[150] flex items-center justify-center pointer-events-auto">
                        ...
                        <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative bg-white w-full max-w-md p-8 shadow-2xl z-10 mx-4 border border-gray-100">
                            <h2 className="text-2xl font-black text-black tracking-tight mb-6">Change Password</h2>
            ...
            <AnimatePresence>
                {isCalendarModalOpen && (
                    <div className="fixed inset-0 z-[150] flex items-center justify-center pointer-events-auto">
                        ...
                        <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative bg-white w-full max-w-lg p-8 shadow-2xl z-10 mx-4 border border-gray-100">
                            <div className="flex items-center justify-between border-b border-gray-100 pb-4 mb-6">
                                <h2 className="text-2xl font-black text-black tracking-tight flex items-center gap-3">
                                    <Calendar className="text-black" size={24} /> Hiring & Availability
                                </h2>
                                <button onClick={() => setIsCalendarModalOpen(false)} className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 text-gray-500 hover:bg-black hover:text-[#FDF22F] transition-colors">
                                                    ...
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        <div>
                                                            <div className="flex items-center gap-2">
                                                                ...
                                                                <span className="text-xs font-black text-black">{iv.applicant?.name}</span>
                                                            </div>
                                                            <p className="text-[11px] text-gray-400 font-bold mt-1 uppercase tracking-tight">
                                                                Role: {iv.applicant?.job_posting?.title || 'System Role'}
                                                            </p>
                                                        </div>
                                                    </div>

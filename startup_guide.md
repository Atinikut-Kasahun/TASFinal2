...
2. **App URL**: Ensure `APP_URL` in `backend/.env` is set to `http://192.168.1.49:8081`.
...
2. Ensure the URL in `src/lib/api.ts` is exactly `http://192.168.1.49:8081/api`.
...
- `APP_URL=http://192.168.1.49:8081`
- `FRONTEND_URL=http://192.168.1.49:3000`
